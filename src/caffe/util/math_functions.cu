...
template <>
void caffe_gpu_axpy_with_bias_ip<fp16>(const int N, const fp16 alpha, const fp16* X,
    fp16* Y, float x_bias, float* y_bias) {
  float* tempX;
  float* tempY;
  cudaMalloc(&tempX, N * sizeof(float));
  ...
  convert_to_float<<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, X, tempX, x_bias);
  convert_to_float_ip<<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, Y, tempY, *y_bias);

  CUBLAS_CHECK(cublasSaxpy(Caffe::cublas_handle(), N, &alpha_float, tempX, 1, tempY, 1));

  *y_bias = 1.0;

  convert_to_fp16_ip<<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, tempY, Y, *y_bias);
  cudaFree(tempX);
  cudaFree(tempY);
}
...
template <>
void caffe_gpu_axpy_half_with_bias_ip(const int N, const float alpha, const fp16* X,
    fp16* Y, float x_bias, float* y_bias) {
  float* tempX;
  float* tempY;
  cudaMalloc(&tempX, N * sizeof(float));
  cudaMalloc(&tempY, N * sizeof(float));
  convert_to_float_ip<<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, X, tempX, x_bias);
  convert_to_float<<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, Y, tempY, *y_bias);
...
template <>
void caffe_gpu_axpy_half_with_bias_ip(const int N, const double alpha, const fp16* X,
    fp16* Y, float x_bias, float* y_bias) {
  double* tempX;
  double* tempY;
  cudaMalloc(&tempX, N * sizeof(double));
