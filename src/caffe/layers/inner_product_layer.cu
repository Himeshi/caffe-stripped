  ...
  if (this->param_propagate_down_[0]) {
    ...
    if (transpose_) {
      caffe_gpu_gemm(CblasTrans, CblasNoTrans,
          K_, N_, M_,
          fp32tofp16((Dtype)1.), bottom_data, top_diff,
          fp32tofp16((Dtype)1.), this->blobs_[0]->mutable_gpu_diff());
    } else {
      caffe_gpu_gemm(CblasTrans, CblasNoTrans,
          N_, K_, M_,
          fp32tofp16((Dtype)1.), top_diff, bottom_data,
          fp32tofp16((Dtype)1.), this->blobs_[0]->mutable_gpu_diff());
    }
  }
  ...
  if (propagate_down[0]) {
    ...
    if (transpose_) {
      caffe_gpu_gemm(CblasNoTrans, CblasTrans,
          M_, K_, N_,
          fp32tofp16((Dtype)1.), top_diff, this->blobs_[0]->gpu_data(),
          fp32tofp16((Dtype)0.), bottom[0]->mutable_gpu_diff());
    } else {
