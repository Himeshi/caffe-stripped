...
void AccuracyLayer<Dtype>::Forward_gpu(
  ...
  if (top.size() == 1) {
    ...
    fp16* counts = bottom[1]->mutable_gpu_diff();
    ...
    Dtype acc;
    caffe_gpu_asum_half(nthreads, acc_data, &acc);
    Dtype valid_count;
    ...
    fp16* counts = nums_buffer_.mutable_gpu_data();

    caffe_gpu_set_half(bottom[0]->count(), Dtype(0), acc_data);
    caffe_gpu_set_half(nums_buffer_.count(), Dtype(0), counts);
    ...
    Dtype acc;
    caffe_gpu_asum_half(bottom[0]->count(), acc_data, &acc);
    Dtype valid_count;
