...
void SplitLayer<Dtype>::Reshape(const vector<Blob<__half>*>& bottom,
...
template <typename Dtype>
void SplitLayer<Dtype>::Forward_cpu(const vector<Blob<__half>*>& bottom,
      const vector<Blob<__half>*>& top) {
  for (int i = 0; i < top.size(); ++i) {
...
template <typename Dtype>
void SplitLayer<Dtype>::Backward_cpu(const vector<Blob<__half>*>& top,
  ...
  for (int i = 2; i < top.size(); ++i) {
    const __half* top_diff = top[i]->cpu_diff();
