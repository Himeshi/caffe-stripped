...
template <typename Dtype>
void ScaleLayer<Dtype>::Reshape(const vector<Blob<fp16>*>& bottom,
      const vector<Blob<fp16>*>& top) {
  const ScaleParameter& param = this->layer_param_.scale_param();
  Blob<fp16>* scale = (bottom.size() > 1) ? bottom[1] : this->blobs_[0].get();
...
template <typename Dtype>
void ScaleLayer<Dtype>::Backward_cpu(const vector<Blob<fp16>*>& top,
