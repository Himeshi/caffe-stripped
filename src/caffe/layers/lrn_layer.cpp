...
void LRNLayer<Dtype>::LayerSetUp(const vector<Blob<fp16>*>& bottom,
  ...
  if (this->layer_param_.lrn_param().norm_region() ==
    ...
    split_layer_->SetUp(bottom, split_top_vec_, bottom_dtype, top_dtype);
    ...
    square_layer_->SetUp(square_bottom_vec_, square_top_vec_, bottom_dtype, top_dtype);
    ...
    pool_layer_->SetUp(square_top_vec_, pool_top_vec_, bottom_dtype, top_dtype);
    ...
    power_layer_->SetUp(pool_top_vec_, power_top_vec_, bottom_dtype, top_dtype);
    ...
    product_layer_->SetUp(product_bottom_vec_, top, bottom_dtype, top_dtype);
  }
}

template <typename Dtype>
void LRNLayer<Dtype>::Reshape(const vector<Blob<fp16>*>& bottom,
...
void LRNLayer<Dtype>::WithinChannelForward(
  ...
  split_layer_->Forward(bottom, split_top_vec_, bottom_dtype, top_dtype);
  square_layer_->Forward(square_bottom_vec_, square_top_vec_, bottom_dtype, top_dtype);
  pool_layer_->Forward(square_top_vec_, pool_top_vec_, bottom_dtype, top_dtype);
  power_layer_->Forward(pool_top_vec_, power_top_vec_, bottom_dtype, top_dtype);
  product_layer_->Forward(product_bottom_vec_, top, bottom_dtype, top_dtype);
}
