...
void ReLULayer<Dtype>::Backward_gpu(const vector<Blob<fp16>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    caffe_expand_blob_ag(bottom[0]->count(), bottom_diff_dtype, bottom_diff, bottom[0]->diff_bias);

    Dtype negative_slope = this->layer_param_.relu_param().negative_slope();
    // NOLINT_NEXT_LINE(whitespace/operators)
    ReLUBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, top_diff_dtype, bottom_data_dtype, bottom_diff_dtype, negative_slope);
    caffe_compress_blob_ag(count, bottom_diff_dtype, bottom_diff, &(bottom[0]->diff_bias));
#ifdef SAMPLE_FLOATS
      if(this->phase_ == TRAIN && this->sample_iter_) {
