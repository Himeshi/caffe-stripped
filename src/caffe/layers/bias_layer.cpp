...
template <typename Dtype>
void BiasLayer<Dtype>::Reshape(const vector<Blob<fp16>*>& bottom,
      const vector<Blob<fp16>*>& top) {
  const BiasParameter& param = this->layer_param_.bias_param();
  Blob<fp16>* bias = (bottom.size() > 1) ? bottom[1] : this->blobs_[0].get();
...
template <typename Dtype>
void BiasLayer<Dtype>::Forward_cpu(const vector<Blob<fp16>*>& bottom,
      const vector<Blob<fp16>*>& top) {
...
template <typename Dtype>
void BiasLayer<Dtype>::Backward_cpu(const vector<Blob<fp16>*>& top,
