...
struct decomposed_posit decompose_posit(fp16 p) {
	...
	if (second_bit) {
		...
		fp16 flipped = ~p;
		regime = __builtin_clz(flipped) - FP16_LIMB_SIZE;
		regime_length = regime + 1;
		regime -= 1;
	} else {
		//sign of regime is -ve, find first 1
		regime = __builtin_clz(p) - FP16_LIMB_SIZE;
		regime_length = regime + 1;
		regime = -regime;
	}
...
fp16 add_posit(fp16 a, fp16 b) {
	...
	uint64_t temp_a, temp_b, fraction;
...
fp16 fused_multiply_add(fp16* a, fp16* b, int count) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (a == 0 || b == 0) {
			...
			//multiply
			struct decomposed_posit a_decomposed = decompose_posit(a_i);
			struct decomposed_posit b_decomposed = decompose_posit(b_i);
